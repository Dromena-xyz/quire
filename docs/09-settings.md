...
- **Version.** The installed version of Quire, with a note when a newer release is
  available.
- **Check for updates.** Quire watches for new releases so an off-directory
  install still stays current. Use this to check on demand.
...
- **Scene template.** A template applied to each new scene note. If you use the
  ...
  own boilerplate.
- **Write scene metadata.** When on, Quire writes a small ordering property into
  each scene's frontmatter. Leave it off to keep scene notes free of any metadata
  you did not add; the index still tracks order either way.
- **Multi-select modifier.** Which key (for example Alt or Cmd) selects more than
  one scene at a time in the list.
